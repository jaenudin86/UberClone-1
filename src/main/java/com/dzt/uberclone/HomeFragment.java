...
public class HomeFragment extends Fragment implements View.OnClickListener, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, GoogleMap.OnMapClickListener, GoogleMap.OnMarkerDragListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if(cd.isConnectedToInternet()==false){

            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
            builder.setMessage("Not connected to Internet. Want to switch it on?")
                    .setCancelable(false)
        ...
        map.setOnMapClickListener(this);//Didnt do this
        map.setOnMarkerDragListener(this);//Didnt do this either
        ...
        {
            //There is something in the sharedPrefs
    ...
    public void onConnected(Bundle bundle) {
        ...
        {
            if (centerOnCurrent)//Which is true if no location in sharedPrefs. Or else false
            {
            ...
            map.addMarker(new MarkerOptions().position(latLng).title("You are here!").draggable(true)); //.icon(BitmapDescriptorFactory.fromResource(R.drawable.car_icon_top)).anchor(0.5,0.5)

            //http://maps.googleapis.com/maps/api/geocode/json?latlng=17.36,76.32&sensor=false
    ...
    private void setLocationName(String json)
    {
        ...
        selectPickup.setText(locationname);//Setting the textView to the new location text string
        getNearbyUbers();
    }

    public void getNearbyUbers()
    {
        //http://192.168.1.76:8000/drivers/get/nearby?latitude=14.32&longitude=73.13&radius=0.005
    ...
    private void showUberMarkers(String json)
    {
        try
        {
            markers.clear();//Any previous markers available would be cleared
            shortestTime = 0;
    ...
    {
//      http://maps.googleapis.com/maps/api/directions/json?origin=12.934873,77.534427&destination=12.945246,77.527990&mode=driving&sensor=false
    ...
    private void showShortestTime(String json)
    {
        try
        {
            ...
            shortestTime += shortestTimeTemp;//Adding all the uber distances?
            ubercount++;
    ...
    //////Not triggered until you change the location manually
    @Override
    public void onMapClick(LatLng latLng) {
        //put a pin and change direction.The LatLng object specifies the location of new pin
        currentLocation = latLng;

        map.moveCamera(CameraUpdateFactory.newLatLng(latLng));

        map.animateCamera(CameraUpdateFactory.zoomTo(16));
        map.clear();//Remove the existing markers for the previous location and start afresh
...
//              This is where the ride id will be put on the cloud
    ...
    private void postSendUberRequest(String json)
    {
        ...
        //This should be an automatic process. Not that the uber driver should accept.
        //Like 20 seconds or so automatic timer
    ...
    {
        //Thread is used to run the process in background
                ...
                public void handleMessage(Message msg) {
                    ...
                    if ((null != aResponse))
                    {
                        ...
                        else
                        {
                            ...
                            syncWithServer = false;//Stop the syncing of data in the background
                        }
                    }
                    else
                    {
    ...
    private void trackDriver()
    {
        ...
        Thread trackdriverthread = new Thread(new Runnable() {
            ...
            public void run() {
                while(true)
                {
                    if (trackDriverBoolean)//Check when the ride will end!
                    {
